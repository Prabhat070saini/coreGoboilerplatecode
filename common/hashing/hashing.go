...
func HashPassword(password *string, saltRound *int) (*string, error) {
	hashBytes, err := bcrypt.GenerateFromPassword([]byte(*password), *saltRound)
	if err != nil {
		return nil, err
	}
...
func EncryptValue(value interface{}, secretKey *string) (*string, error) {
	key := []byte(*secretKey)
	if len(key) != 32 {
		return nil, errors.New("secret key must be 32 bytes for AES-256")
	}
